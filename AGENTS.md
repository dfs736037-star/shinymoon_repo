...
| `shinymoon-code-agent` | Auto-route coding to `shinymoon-code` subagent + MCP/graphify |
...
| `ollama` | Local LLM offload (summaries, commits, drafts) — see `.cursor/setup/OLLAMA_INTEGRATION.md` |
...
Default executor: **`shinymoon-code`** (`.cursor/agents/shinymoon-code.md`). Invoke with `/code <task>` or `/shinymoon-code <task>`. Automatically loads skills, `shinymoon-alpha-tools` MCP, and graphify.

## File rules

- Edit `shinymoon_alpha.lua` only unless user names another file.
- Keep `def_voice_decoders.lua` at repo root (Neverlose `require`).
- Legacy reference scripts live in `reference/legacy/` — read-only unless user asks.
