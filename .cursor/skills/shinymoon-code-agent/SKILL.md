...
**Delegate to the `shinymoon-code` subagent** (`.cursor/agents/shinymoon-code.md`) for any task that reads or writes project code.
...
3. MCP `shinymoon-alpha-tools`: docs + `search_project`
4. ponytail minimal diff
5. plan-first gate for large features
6. `graphify update .` after structural edits

See `.cursor/agents/shinymoon-code.md` for the full decision tree.
